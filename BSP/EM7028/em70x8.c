...
uint8_t EM7028_Get_ID()//获取EM7028芯片的ID，以验证通讯是否正常。
{
	return EM7028_ReadOneReg(ID_REG);
}

uint8_t EM7028_hrs_init()//初始化EM7028芯片的心率监测模块，设置相应的寄存器配置，包括开启心率测量、设置增益、设置频率
{
	uint8_t i = 5;
...
uint8_t EM7028_hrs_DisEnable()//禁用
{
	uint8_t i = 5;
	while(EM7028_Get_ID() != 0x36 && i)
	{
...
uint16_t EM7028_Get_HRS1(void)//获取HRS1通道的心率测量结果
{
	uint16_t dat;
