...
uint8_t ChargeCheck()//1:充电PA2
{
	return HAL_GPIO_ReadPin(CHARGE_PORT,CHARGE_PIN);
}

float BatCheck()//计算电池电压PA1
{
	uint16_t dat;
	float BatVoltage;
	HAL_ADC_Start(&hadc1);
...
uint8_t PowerCalculate()        //电量转换百分比
{
	uint8_t power;
	float voltage;
	voltage = BatCheck_8times();
	
	if(ChargeCheck())                           //如果在充电
...
void Power_Init(void)       //使能芯片TPS63020
{
	Power_Pins_Init();
	Power_Enable();
}
